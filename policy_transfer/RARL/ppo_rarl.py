...
from design_optimize.design_net import *
...
def traj_segment_generator(pi, env, horizon, stochastic, rarl_pi):
    t = 0
    ...
    if rarl_pi is not None:
        rarl_ac = env.env.env.learnable_perturbation_space.sample()
        rarl_acs = np.array([rarl_ac for _ in range(horizon)])
    else:
        rarl_acs = []
    ...
    while True:
        ...
        if rarl_pi is not None:
            rarl_ac, _ = rarl_pi.act(stochastic, ob)
        ...
        if t > 0 and t % horizon == 0:
            yield {"ob" : obs, "rew" : rews, "vpred" : vpreds, "new" : news,
                    ...
                    "ep_rets" : np.array(ep_rets), "ep_lens" : ep_lens, "avg_vels":avg_vels,
                   "rarl_ac":np.array(rarl_acs)}
        ...
        if rarl_pi is not None:
            rarl_acs[i] = rarl_ac
            env.env.env.learnable_perturbation_act = rarl_ac
...
def learn(env, policy_func, *,
    ...
    loss_input = [ob, ac, atarg, ret, lrmult, clip_tf]
    lossandgrad = U.function(loss_input, losses + [U.flatgrad(total_loss, var_list)])
    ...
    compute_losses = U.function(loss_input, losses)
    ...
    if rarl:
        ...
        surr1 = rarl_ratio * atarg  # surrogate from conservative policy iteration
        surr2 = tf.clip_by_value(rarl_ratio, 1.0 - clip_tf, 1.0 + clip_tf * lrmult) * atarg
        rarl_pol_surr = tf.reduce_mean(tf.minimum(surr1, surr2))   # the rarl_policy is acting ADVERSARIALLY

        rarl_var_list = rarl_pi.get_trainable_variables()
        loss_input = [ob, rarl_ac, atarg, ret, lrmult, clip_tf]
        rarl_lossandgrad = U.function(loss_input, [U.flatgrad(rarl_pol_surr, rarl_var_list)])
        rarl_adam = MpiAdam(rarl_var_list, epsilon=adam_epsilon)
        rarl_assign_old_eq_new = U.function([], [], updates=[tf.assign(oldv, newv)
                                                        for (oldv, newv) in
                                                        zipsame(rarl_oldpi.get_variables(), rarl_pi.get_variables())])
    ...
    # ----------------------------------------
    if not rarl:
        seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, rarl_pi=None)
    else:
        seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, rarl_pi = rarl_pi)
    ...
    while True:
        ...
        for epoch in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                comp_input = [batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult, cur_clip_val]
                *newlosses, g = lossandgrad(*comp_input)
        ...
        if rarl:
            rarl_d = Dataset(dict(ob=ob, rarl_ac=seg['rarl_ac'], atarg=atarg, vtarg=tdlamret), shuffle=not pi.recurrent)
            for epoch in range(optim_epochs):
                losses = []  # list of tuples, each of which gives the loss for a minibatch
                for batch in rarl_d.iterate_once(optim_batchsize):
                    comp_input = [batch["ob"], batch["rarl_ac"], batch["atarg"], batch["vtarg"], cur_lrmult, cur_clip_val]
                    *newlosses, g = rarl_lossandgrad(*comp_input)
                    rarl_adam.update(g * grad_scale, optim_stepsize * 0.05)
                    losses.append(newlosses)
        ...
        for batch in d.iterate_once(optim_batchsize):
            comp_input = [batch["ob"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult, cur_clip_val]
            newlosses = compute_losses(*comp_input)
            losses.append(newlosses)
